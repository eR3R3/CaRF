...
def render_sets(dataset : ModelParams,model_path, pipeline : PipelineParams, skip_train : bool, skip_test : bool, args):
    with torch.no_grad():  
        ...
        checkpoint = model_path  # model_path is already the full checkpoint path
        ...
        if not skip_test:
             render_set(dataset.model_path, dataset.source_path, "testccc", 1, scene.getTestCameras(), gaussians, pipeline, background, args)
...
if __name__ == "__main__":
    ...
    # 新增：checkpoint 路径（文件或目录）
    parser.add_argument("-c", "--checkpoint", type=str, required=True,
                        help="checkpoint 的路径：可以是具体 .pth 文件，也可以是包含 chkpnt_iter_*.pth 的目录")
    ...
    if os.path.isdir(ckpt_path):
        if args.iteration < 0:
            raise ValueError("当 --checkpoint 指向目录时，请同时指定 --iteration >= 0。")
        ckpt_path = os.path.join(ckpt_path, f"chkpnt_iter_{args.iteration}.pth")
    
    render_sets(model.extract(args), ckpt_path, pipeline.extract(args), args.skip_train, args.skip_test, args)
